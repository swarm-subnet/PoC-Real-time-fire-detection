...
class CameraSnapshot:
    """Latest camera frames plus monotonic frame versions.

    With copy=False, frames are shared read-only references. Callers must not
    mutate them; camera threads replace frame objects instead of editing them.
    """
...
class SwarmCameraWall:
    ...
    def get_frame_versions(self) -> dict[str, int]:
        """Return monotonically increasing versions for real decoded frames only."""
        with self._lock:
            return dict(self.frame_versions)

    def get_snapshot(self, copy: bool = True) -> CameraSnapshot:
        """Return latest frames and versions from the same lock acquisition."""
        with self._lock:
            frames = dict(self.frames)
            for ip in self.ips:
                frames.setdefault(ip, self._placeholder(ip, self.errors.get(ip, "no frame")))
    ...
    def _stop_drone_stream(self, ip: str, wait_response: bool = False, timeout: float = 0.5) -> None:
        try:
            if wait_response:
