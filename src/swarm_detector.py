...
REQUIRED_CUDA_DEVICE = "0"
REQUIRED_CUDA_HALF = True
...
def _ultralytics_windows_os_release_guard():
    """Make Ultralytics Linux-distro probes safe under Windows Python on WSL UNC paths."""
    if os.name != "nt":
        yield
        return
...
class SwarmPersonDetector:
    """Runs one shared YOLO model over latest drone frames in a background loop."""
        ...
        with self._lock:
            for ip, frame in frames.items():
                if ip not in self.ips or frame is None:
                    continue
                if frame_versions is not None:
                    source_version = frame_versions.get(ip, 0)
                    if source_version <= 0 or source_version <= self._frame_versions.get(ip, 0):
                        continue
                    self._frame_versions[ip] = source_version
                else:
                    self._frame_versions[ip] = self._frame_versions.get(ip, 0) + 1
    ...
    def snapshot_stats(self) -> PersonDetectorStats:
        with self._lock:
            ...
            overall_fps = self._total_inferences / elapsed if elapsed > 0 else 0.0
    ...
    def ensure_ready(self) -> None:
        """Load YOLO and prove a CUDA inference path before the dashboard starts."""
        try:
    ...
    def _run_loop(self) -> None:
        try:
            self._ensure_model_loaded()
            self._set_status(f"running:{self._resolved_device}")
        except Exception as error:
            self._set_error(f"detector unavailable: {error}")
            return
    ...
    def _ensure_model_loaded(self) -> None:
        ...
        self._verify_model_inference(model, device)

        with self._lock:
            self._resolved_device = device
            self._model = model
            self._loaded = True
            self._status = f"ready:{device}"
            self._last_error = ""
    ...
    def _resolve_device(self) -> str:
        ...
        if not torch.cuda.is_available():
            raise RuntimeError("CUDA GPU is required for person detection; torch.cuda.is_available() is False")
        if torch.cuda.device_count() < 1:
            raise RuntimeError("CUDA GPU is required for person detection; torch.cuda.device_count() is 0")
        return REQUIRED_CUDA_DEVICE

    def _use_half_precision(self) -> bool:
        return REQUIRED_CUDA_HALF and self._resolved_device == REQUIRED_CUDA_DEVICE

    def _verify_model_inference(self, model, device: str) -> None:
        try:
            import torch
        except Exception as error:
            raise RuntimeError("PyTorch with CUDA is required for person detection") from error
        ...
        if module is not None and hasattr(module, "parameters"):
            ...
            if first_param is not None and not first_param.is_cuda:
                raise RuntimeError("YOLO model did not remain on CUDA after inference")

    def _get_next_batch(self) -> tuple[list[str], list[np.ndarray], list[int]] | None:
        ips: list[str] = []
        frames: list[np.ndarray] = []
        versions: list[int] = []
        with self._lock:
            for ip in self.ips:
                version = self._frame_versions.get(ip)
                if version is None or self._processed_versions.get(ip) == version:
                    continue
    ...
    def _process_batch(self, ips: list[str], frames: list[np.ndarray], versions: list[int]) -> None:
        ...
        try:
            ...
            for ip, frame, result in zip(ips, frames, results):
                detections_by_ip[ip] = self._parse_result(ip, frame, result)
            self._record_batch_result(ips, versions, detections_by_ip, inference_ms)
        except Exception as error:
            self._set_error(f"detector error: {error}")
            time.sleep(0.25)

    def _wait_for_inference_slot(self) -> bool:
        max_batches = float(self.config.max_batches_per_second)
        if max_batches <= 0:
            return True
    ...
    def _parse_result(self, ip: str, frame: np.ndarray, result) -> list[PersonDetection]:
        ...
        for xyxy, confidence in zip(xyxy_values, conf_values):
            values = np.asarray(xyxy, dtype=np.float32).reshape(-1)
            confidence_value = float(confidence)
            if values.shape[0] != 4:
                continue
            if not np.all(np.isfinite(values)) or not np.isfinite(confidence_value):
                continue
    ...
    def _record_batch_result(
        ...
        with self._lock:
            for ip, version in zip(ips, versions):
                # Keep the processed marker current even if frames arrive faster than inference.
                ...
                if detections:
                    ...
                    previous = self._detections.get(ip, [])
                    self._detections[ip] = [
                        detection
            ...
            if self._average_inference_ms is None:
                self._average_inference_ms = inference_ms
            else:
                self._average_inference_ms = 0.9 * self._average_inference_ms + 0.1 * inference_ms
            self._status = f"running:{self._resolved_device}"
            self._last_error = ""
